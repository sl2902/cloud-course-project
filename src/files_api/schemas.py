...
class GeneratedFilesQueryParams(BaseModel):
    """Query parameters for `POST /v1/files/generated/chat/completion`."""
    ...
    def validate_file_path_extension(self) -> Self:
        ...
        # if file_type == GeneratedFileType.TEXT and not self.file_path.endswith(".txt"):
        #     raise ValueError("For text files, the path must end with .txt")
        # if file_type == GeneratedFileType.IMAGE and \
        #     not re.match(r".*\.(png|jpg|jpeg)$", self.file_path):
        #     raise ValueError("For image files, the path must end with .png, .jpg, or .jpeg")
        # if file_type == GeneratedFileType.AUDIO and \
        #     not re.match(r".*\.(mp3|opus|aac|flac|wav|pcm)$", self.file_path):
        #     raise ValueError("For audio files, the path must end with .mp3, .opus, .aac, .flac, .wav, or .pcm")
        if not self.file_path.endswith(".txt"):
            raise ValueError("For text files, the path must end with .txt")
        # if not re.match(r".*\.(png|jpg|jpeg)$", self.file_path):
        #     raise ValueError("For image files, the path must end with .png, .jpg, or .jpeg")
        # if not re.match(r".*\.(mp3|opus|aac|flac|wav|pcm)$", self.file_path):
        #     raise ValueError("For audio files, the path must end with .mp3, .opus, .aac, .flac, .wav, or .pcm")
        
        return self

class GeneratedImagesQueryParams(BaseModel):
    """Query parameters for `POST /v1/files/generated/image/generation`."""
